...
import { Plus, Edit3, Trash2, Banknote, Package, DollarSign } from "lucide-react"
import { Label } from "@/components/ui/label"
import { toast } from "sonner"
import { useDispatch } from "react-redux"
import type { AppDispatch } from "@/lib/store"

// Mock data for banks and platform settings
const initialBanks = [
  {
    id: "zb",
    name: "ZB Bank",
...
export default function AdminFinancePage() {
  ...
  const [newBank, setNewBank] = useState({ name: "", logo: "", interestRate: "", minAmount: "", maxAmount: "", status: "active" as "active" | "inactive" })
  ...
  const addBank = () => {
    ...
    const newBankData = { ...newBank, id: bankId }
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h1 className="text-4xl font-light tracking-tight">Finance Configuration</h1>
        <p className="text-muted-foreground text-lg mt-2 max-w-2xl">
          Manage banks available for seller/buyer loans, shipping rates and platform fees. Changes sync automatically.
        </p>
      </motion.div>
                    ...
                    {banks.map((bank) => (
                      <TableRow key={bank.id} className="border-b border-border/50 hover:bg-accent/5">
                        ...
                        <TableCell className="text-right space-x-1">
                          ...
                          <Button variant="ghost" size="icon" className="text-destructive hover:text-destructive hover:bg-destructive/10" onClick={() => deleteBank(bank.id)}>
