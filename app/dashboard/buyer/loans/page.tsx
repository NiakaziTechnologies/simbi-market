...
import { fetchLoansAsync } from "@/lib/api/loans"
...
export default function BuyerLoansPage() {
  ...
  useEffect(() => {
    dispatch(fetchLoansAsync('buyer')).unwrap()
  }, [dispatch])
    ...
    <div className="max-w-6xl mx-auto space-y-6 sm:space-y-8 p-4 sm:p-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 lg:gap-6">
          <div className="flex-1">
            ...
            <p className="text-muted-foreground font-light max-w-2xl text-sm sm:text-base lg:text-lg mt-2">
              Financing for premium auto parts purchases. Instant eligibility based on credit profile.
            </p>
          </div>
      ...
      >
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="flex w-full overflow-x-auto gap-1 sm:gap-0 sm:grid sm:grid-cols-4 bg-gradient-to-r from-slate-900/20 to-slate-800/20 backdrop-blur-md rounded-2xl p-1 border border-accent/20 shadow-xl">
              <TabsTrigger value="apply" className="flex-shrink-0 sm:flex-shrink whitespace-nowrap px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-lg text-xs sm:text-sm data-[state=active]:bg-blue-500/90 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-blue-500/25 transition-all">
                <Banknote className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="ml-1.5 sm:ml-2">Apply New</span>
              </TabsTrigger>
              <TabsTrigger value="approved" className="flex-shrink-0 sm:flex-shrink whitespace-nowrap px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-lg text-xs sm:text-sm data-[state=active]:bg-green-500/90 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-green-500/25 transition-all">
                <TrendingUp className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="ml-1.5 sm:ml-2">Approved ({stats.approved})</span>
              </TabsTrigger>
              <TabsTrigger value="pending" className="flex-shrink-0 sm:flex-shrink whitespace-nowrap px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-lg text-xs sm:text-sm data-[state=active]:bg-amber-500/90 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-amber-500/25 transition-all">
                <CreditCard className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="ml-1.5 sm:ml-2">Pending ({stats.pending})</span>
              </TabsTrigger>
              <TabsTrigger value="rejected" className="flex-shrink-0 sm:flex-shrink whitespace-nowrap px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl sm:rounded-lg text-xs sm:text-sm data-[state=active]:bg-destructive/90 data-[state=active]:text-white data-[state=active]:shadow-lg data-[state=active]:shadow-destructive/25 transition-all">
                <AlertCircle className="h-3.5 w-3.5 sm:h-4 sm:w-4" />
                <span className="ml-1.5 sm:ml-2">Declined ({stats.rejected})</span>
              </TabsTrigger>
            </TabsList>

          <TabsContent value="apply" className="mt-6 sm:mt-8">
            <LoanForm type="buyer" score={eligibilityScore} />
          </TabsContent>
      ...
      >
        <p>Loans provided by partner banks • Subject to approval • Simbi Market is a financing facilitator</p>
      </motion.div>
    </div>
  )
}
